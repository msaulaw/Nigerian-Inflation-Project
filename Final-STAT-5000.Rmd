---
title: "Effects of Economic Factors on Rates of Violence in Nigeria; Final STAT 5000"
author: "Maryam Saulawa"
date: "`r Sys.Date()`"
...
I chose this topic because I am Nigerian and I saw a graph of the violence presented by the economist which made me realize that the violence might be connected to rising inflation and general food prices.
...
I want to see if violence in Nigeria is in any way correlated with the rise in inflation and food prices as well as the FX rate changes.
...
I began by checking the data and getting rid of the months, years, and date to replace with a time series formula. I wanted to avoid dealing with null values, so I began the time series from the year 2004 and ended it at October 2024.
...
I then plotted the variables on a single graph. the variables I looked at were the value of Dollar against the currency Naira, food inflation and general inflation, which are the small lines, and the total acts of political violence each month.

```{r plot, echo=FALSE}
ts_plot(tseries, title = "Nigeria", Xtitle = "Date"      )
```

I also plotted each variable separately to better view the patterns of the series. I then saw that there was generally an increase in each category.
...
I then decided to use a linear regression model to see if inflation, food inflation, usd rates had any effect on the rate of violence the occurred. The overall model had an r^{2} of 77% and a small p-value, which is decent, but not the best. From the model, I gathered the by p-value, t-value, and standard error , USD increases had a positive effect of rate of violence, which is bad, and I hope something changes with the way they are handling the currency and the other variables that are affecting the rate of violence in the country. It was, however, somewhat a surprise that the model showed a a negative relationship with inflation. This is surprising the me since it has been reported that Nigerians spend 96% of their income on food supplies. I don't know how to explain that, but I do know from my family, that the value of dollar is followed closely by merchants and sellers of various items.
...
I first decomposed the variables together and seperately. (idk why)
...
I performed the Portmanteau test and saw there was no autocorrelation. The ARCH test shows there is conditional heteroscedasticity. The normality test showed that the data is not normally distributed and residuals aren't all in the confidence intervals. 
...
Afterwards, I forcasted a year into the future. I tried the `predict()` function and the `forecast()` function.
...
# Conclusion
After reviewing the plots of the ARIMA model, I see that violence will rise, but not very drastically. The value of Dollar against Naira seems to drop dramatically in the next 12 months which might be helpful to ward off violence. Both inflation variables  show an overall constant growth that might be eased if Dollar actually drops.
